...
void findSwitchAndCase(string str);
...
void searchKey(string str){//查找可能是的情况 
...
bool compareWithKeys(string str1, string str2){//对比str和key，进行确认 
	...
	if(str.length() < sizeof(key[2])){
		...
		for(i = 0;i < str.length() - 4;i++){
			for(j = i;(j - i) < 4;j++){
				if(str[j] == key[2][c]){
					if(c == 3){
						case_num[case_p]++;//在已知的switch状态下的case数量增加 
					}
					c++;
				}else{
					break;
